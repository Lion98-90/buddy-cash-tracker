...
=========================================================
  ...
  const handleDownloadReport = () => {
    ...
    const doc = new jsPDF('p', 'pt', 'a4'); // Use points and A4; 'pt' is jsPDF default unit
    ...
    const lineSpacing = baseFontSize * 1.4; // Adjusted line spacing
    ...
    const checkAndAddPage = (currentY: number, neededHeight: number = 0) => {
      if (currentY + neededHeight > pageHeight - margin) { // Check against bottom margin
        doc.addPage();
        return margin; // Reset yPos for new page to top margin
      }
      return currentY;
    };
    ...
    sections.forEach((section) => {
      yPos = checkAndAddPage(yPos, sectionSpacing); // Ensure space before new section header
      ...
      if (header === "BUDDYCASH FINANCIAL REPORT") {
        // Add generated date and currency if they were under the main title
      ...
      if (header) {
        yPos = checkAndAddPage(yPos, headerFontSize * 1.2); // Space for header itself
      ...
      if (header === "EXECUTIVE SUMMARY" || header === "CONTACT SUMMARY") {
        ...
        // This is a super-header, actual tables are "PEOPLE WHO OWE YOU" and "PEOPLE YOU OWE"
        // No specific text content for this header itself, tables will follow.
        ...
        if (tableData.length > 0) {
          autoTable({
            ...
            didDrawPage: () => { yPos = margin; } // Reset yPos if page break by autoTable
          });
        ...
        // This is handled by the global footer logic now
        return;
      } else {
         // Fallback for any other text not caught
        sectionLines.forEach(line => {
          if (line.trim()) {
            yPos = checkAndAddPage(yPos, lineSpacing);
            doc.text(line.trim(), margin, yPos, { maxWidth: pageWidth - margin * 2 });
    ...
    for (let i = 1; i <= numPages; i++) {
      doc.setPage(i);
      yPos = checkAndAddPage(yPos); // Ensure we're not writing off page
