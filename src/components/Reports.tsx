...
SUMMARY STATISTICS
==================
${summaryStats.map(stat => `${stat.title}: ${stat.value}`).join('\n')}

MONTHLY BREAKDOWN
=================
${monthlyData.map(month => 
  `${month.month}: Given ${currencySymbol}${month.given}, Received ${currencySymbol}${month.received}`
).join('\n')}

TOP PEOPLE (THEY OWE YOU)
========================
${topOwedToYou.length > 0 ? 
  topOwedToYou.map(person => `${person.name}: ${currencySymbol}${person.amount.toFixed(0)}`).join('\n') : 
  'No one owes you money'
}

TOP PEOPLE (YOU OWE THEM)
========================
${topYouOwe.length > 0 ? 
  topYouOwe.map(person => `${person.name}: ${currencySymbol}${person.amount.toFixed(0)}`).join('\n') : 
...
${filteredTransactions.slice(0, 20).map(t => 
  `${t.date.split('T')[0]} - ${t.type === 'given' ? 'Given' : 'Received'} ${currencySymbol}${Math.abs(t.amount)} - ${t.description || 'No description'}`
).join('\n')}

Generated by BuddyCash App
    `.trim();
  ...
  const handleExportReport = () => {
    ...
    a.href = url;
    a.download = `buddycash-financial-report-${dateRange}-${new Date().toISOString().split('T')[0]}.txt`;
    a.click();
    window.URL.revokeObjectURL(url);
    
    toast({
      title: "Report Exported",
      description: "Your financial report has been exported as a readable text file",
    });
  };
